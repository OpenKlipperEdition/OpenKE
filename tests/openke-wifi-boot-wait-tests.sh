...
#
# Usage: sh tests/openke-wifi-boot-wait-tests.sh

set -u

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
LIB="$SCRIPT_DIR/../scripts/build/overlay/etc/openke-wifi-boot-wait.sh"
WORK=$(mktemp -d "${TMPDIR:-/tmp}/openke-wifi-boot-wait-tests.XXXXXX")
...
OPENKE_WIFI_BOOT_WAIT_MIN=0
OPENKE_WIFI_BOOT_WAIT_TIMEOUT=2
...
mode=$(openke_wifi_boot_wait_requested_mode "$MARKER")
if [ "$mode" = "fixed" ]; then
	pass
else
...
PATH="$FAKE_BIN:$PATH" openke_wifi_wait_for_association_event_driven testif >/dev/null 2>&1
rc=$?
...
chmod +x "$FAKE_BIN/wpa_cli"
start_s=$(date +%s 2>/dev/null || echo 0)
if PATH="$FAKE_BIN:$PATH" openke_wifi_wait_for_association_event_driven testif >/dev/null 2>&1; then
	fail "wait reported success even though association never completed"
else
	end_s=$(date +%s 2>/dev/null || echo 0)
	elapsed=$((end_s - start_s))
	if [ "$elapsed" -ge "$OPENKE_WIFI_BOOT_WAIT_TIMEOUT" ] && [ "$elapsed" -le $((OPENKE_WIFI_BOOT_WAIT_TIMEOUT + 2)) ]; then
		pass
	else
		fail "wait did not respect the configured timeout bound (elapsed=${elapsed}s, timeout=${OPENKE_WIFI_BOOT_WAIT_TIMEOUT}s)"
	fi
fi
...
: > "$FAKE_WPA_CALLED"
OPENKE_WIFI_BOOT_WAIT_MARKER="$WORK/marker-absent-2"
PATH="$FAKE_BIN:$PATH" openke_wifi_boot_wait testif >/dev/null 2>&1
...
: > "$FAKE_WPA_CALLED"
OPENKE_WIFI_BOOT_WAIT_MARKER="$WORK/marker-event-2"
printf 'event-driven\n' > "$OPENKE_WIFI_BOOT_WAIT_MARKER"
PATH="$FAKE_BIN:$PATH" openke_wifi_boot_wait testif >/dev/null 2>&1
if [ -s "$FAKE_WPA_CALLED" ]; then
	pass
else
