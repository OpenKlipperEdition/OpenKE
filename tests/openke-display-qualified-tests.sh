...
# tests/openke-display-sleep-wake-controller-tests.sh - kept apart the
# same way tests/openke-camera-idle-controller-tests.sh is kept apart
...
#
# Usage: sh tests/openke-display-qualified-tests.sh

set -u

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/.." && pwd)
LIB="$REPO_ROOT/scripts/build/overlay/etc/openke-display-qualified.sh"
HEALTHCHECK_LIB="$REPO_ROOT/scripts/build/overlay/etc/openke-healthcheck.sh"
APPLY_SCRIPT="$REPO_ROOT/scripts/build/overlay/etc/init.d/S97openke-display-qualified-apply"
WRITE_HELPER="$REPO_ROOT/scripts/build/overlay/usr/libexec/openke-display-qualified-write"
INITD_DIR="$REPO_ROOT/scripts/build/overlay/etc/init.d"

WORK=$(mktemp -d "${TMPDIR:-/tmp}/openke-display-qualified-tests.XXXXXX")
...
echo "============================================================"
echo "Write helper (usr/libexec/openke-display-qualified-write)"
echo "============================================================"
...
i_ours=$(index_of S97openke-display-qualified-apply)
i_sleepwake=$(index_of S98openke-display-sleep-wake-controller)
i_confirmgood=$(index_of S99confirm-good)

expected_scripts="i_klipper:$i_klipper i_moonraker:$i_moonraker i_guppy:$i_guppy i_mcurecovery:$i_mcurecovery i_ours:$i_ours i_sleepwake:$i_sleepwake"
if [ -n "$i_confirmgood" ]; then
	expected_scripts="$expected_scripts i_confirmgood:$i_confirmgood"
fi

for name_idx in $expected_scripts; do
	val=${name_idx#*:}
	if [ -z "$val" ]; then
...
if [ -n "$i_klipper" ] && [ -n "$i_moonraker" ] && [ -n "$i_guppy" ] && [ -n "$i_ours" ]; then
...
SWC_LIB="$REPO_ROOT/scripts/build/overlay/etc/openke-display-sleep-wake-controller.sh"
