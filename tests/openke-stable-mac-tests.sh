...
#
# Usage: sh tests/openke-stable-mac-tests.sh

set -u

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
LIB="$SCRIPT_DIR/../scripts/build/overlay/etc/openke-stable-mac.sh"
WORK=$(mktemp -d "${TMPDIR:-/tmp}/openke-stable-mac-tests.XXXXXX")
...
mac_a=$(openke_derive_mac_from_identifier "0011223344556677889900112233445566")
mac_b=$(openke_derive_mac_from_identifier "0011223344556677889900112233445566")
...
mac_c=$(openke_derive_mac_from_identifier "ffeeddccbbaa99887766554433221100ff")
if [ "$mac_a" != "$mac_c" ]; then
	pass
else
...
while [ "$i" -lt 20 ]; do
	...
	m=$(openke_derive_mac_from_identifier "$ident")
	case " $prev_macs " in
		*" $m "*) collision=1 ;;
	esac
...
if empty_mac=$(openke_derive_mac_from_identifier ""); then
...
if hw_id=$(openke_read_hardware_identifier 2>/dev/null); then
	fail "openke_read_hardware_identifier unexpectedly succeeded in a non-hardware test environment (got '$hw_id')"
else
	pass
fi
...
fb1=$(openke_read_or_create_fallback_identifier "$SEED_FILE")
fb2=$(openke_read_or_create_fallback_identifier "$SEED_FILE")
...
fb3=$(openke_read_or_create_fallback_identifier "$SEED_FILE_2")
...
fb4=$(openke_read_or_create_fallback_identifier "$SEED_FILE_3")
case "$fb4" in
...
# proves openke_stabilize_iface_mac derives a MAC and attempts to apply
...
export FAKE_IP_LOG
: > "$FAKE_IP_LOG"
OPENKE_MAC_SEED_FILE="$WORK/wifi-mac-seed-e2e"
export OPENKE_MAC_SEED_FILE
if PATH="$FAKE_BIN:$PATH" openke_stabilize_iface_mac testif >/dev/null 2>&1; then
	...
	fail "openke_stabilize_iface_mac failed end-to-end with a working fake ip and a fresh seed file"
fi
...
# (/usr/data/openke/wifi-mac-seed) lives on the persistent partition,
# never inside anything this build tracks in git. ---
if grep -rEq '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' "$LIB" "$SCRIPT_DIR/../scripts/build/overlay/etc/init.d/S01wifi"; then
