...
# scenario list.
#
# Usage: sh tests/openke-camera-idle-controller-tests.sh

set -u

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
LIB="$SCRIPT_DIR/../scripts/build/overlay/etc/openke-camera-idle-controller.sh"
WORK=$(mktemp -d "${TMPDIR:-/tmp}/openke-camera-idle-tests.XXXXXX")
...
OPENKE_CAMERA_IDLE_GRACE_SAMPLES=3
OPENKE_CAMERA_IDLE_RESUME_RETRIES=2
OPENKE_CAMERA_IDLE_RESUME_BACKOFF_BASE=0
...
EOF
if openke_camera_has_established_connection 8080 "$WORK/proc-net-tcp-one"; then
	pass
else
...
EOF
if openke_camera_has_established_connection 8080 "$WORK/proc-net-tcp-multi"; then
	pass
else
...
EOF
if openke_camera_has_established_connection 8080 "$WORK/proc-net-tcp-none"; then
	fail "a LISTEN-state (not ESTABLISHED) row on port 8080 was incorrectly treated as an active viewer"
else
	pass
fi

if openke_camera_has_established_connection 8080 "$WORK/nonexistent-proc-file" >/dev/null 2>&1; then
...
state=active; idle=0
i=1
while [ "$i" -le "$OPENKE_CAMERA_IDLE_GRACE_SAMPLES" ]; do
	result=$(openke_camera_idle_tick "$state" "$idle" no)
...
if [ "$state" = "paused" ] && grep -q "/pause" "$FAKE_CURL_LOG"; then
	...
	fail "state after $OPENKE_CAMERA_IDLE_GRACE_SAMPLES idle ticks was '$state', expected paused (log: $(cat "$FAKE_CURL_LOG"))"
fi
...
result=$(openke_camera_idle_tick "$state" "$idle" no)
state=${result%% *}; idle=${result##* }
result=$(openke_camera_idle_tick "$state" "$idle" yes)
state=${result%% *}; idle=${result##* }
...
# ============================================================
reset_curl_log
result=$(openke_camera_idle_tick paused 0 yes)
state=${result%% *}
...
# ============================================================
reset_curl_log
result=$(openke_camera_idle_tick active 0 yes)
state=${result%% *}; idle=${result##* }
result=$(openke_camera_idle_tick "$state" "$idle" no)
state=${result%% *}; idle=${result##* }
...
state=active; idle=$((OPENKE_CAMERA_IDLE_GRACE_SAMPLES - 1))
result=$(openke_camera_idle_tick "$state" "$idle" no)
state=${result%% *}
...
# ============================================================
result=$(openke_camera_idle_tick paused 0 no)
state=${result%% *}
...
recovered=$(openke_camera_idle_read_state "$STATE_FILE")
...
recovered=$(openke_camera_idle_read_state "$WORK/state-missing")
...
if openke_camera_pause >/dev/null 2>&1; then
	fail "pause() reported success on an HTTP 500 response"
else
	pass
fi
if openke_camera_resume_with_retry >/dev/null 2>&1; then
...
i=1
while [ "$i" -le 5 ]; do
	result=$(openke_camera_idle_tick "$state" 0 no)
	state=${result%% *}
	i=$((i + 1))
done
...
i=1
while [ "$i" -le 5 ]; do
	result=$(openke_camera_idle_tick "$state" 0 yes)
	state=${result%% *}
	i=$((i + 1))
done
