...
# same pattern as tests/openke-camera-idle-controller-tests.sh: sources
...
# convention tests/openke-display-qualified-tests.sh already establishes
# for these exact two debugfs files), never a real kernel.
#
# HONESTY NOTE, same as tests/openke-display-qualified-tests.sh's own:
...
#
# Usage: sh tests/openke-display-sleep-wake-controller-tests.sh

set -u

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/.." && pwd)
LIB="$REPO_ROOT/scripts/build/overlay/etc/openke-display-sleep-wake-controller.sh"
NDQ_LIB_REAL="$REPO_ROOT/scripts/build/overlay/etc/openke-display-qualified.sh"
INITD_SCRIPT="$REPO_ROOT/scripts/build/overlay/etc/init.d/S98openke-display-sleep-wake-controller"

WORK=$(mktemp -d "${TMPDIR:-/tmp}/openke-display-sleep-wake-tests.XXXXXX")
...
(
	NDQ_CONFIG_FILE="$CONF"
	OPENKE_DISPLAY_SLEEP_WAKE_POLL_INTERVAL=0.1
	export NDQ_CONFIG_FILE OPENKE_DISPLAY_SLEEP_WAKE_POLL_INTERVAL
	ndq_swc_run_loop
) &
loop_pid=$!
...
# use (e.g. the faked wget/ip in tests/openke-display-qualified-tests.sh).
FAKE_BIN="$WORK/bin"
mkdir -p "$FAKE_BIN"
