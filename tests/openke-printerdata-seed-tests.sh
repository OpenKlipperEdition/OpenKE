#!/bin/sh
#
# Offline, repeatable tests for the OpenKE printer_data/config factory
...
#
# Sources scripts/build/overlay/etc/init.d/S02openke-namespace directly
# (S02OPENKE_NAMESPACE_NO_AUTORUN=1, same convention as
# tests/factory-seed-git-tests.sh's S04OPENKE_FACTORY_SEED_NO_AUTORUN)
# with OPENKE_ROOT/PRINTER_DATA_CONFIG_SEED pointed at fixture
# directories - never touches a real device or /usr/data.
#
# Usage: sh tests/openke-printerdata-seed-tests.sh
...
# actually produces at /opt/openke-seeds/printer_data-config/).
build_seed_source() {
	dir="$1"
	rm -rf "$dir"
...
run_seed() {
	# $1=OPENKE_ROOT fixture $2=PRINTER_DATA_CONFIG_SEED fixture
	(
		OPENKE_ROOT="$1"
		PRINTER_DATA_CONFIG_SEED="$2"
		S02OPENKE_NAMESPACE_NO_AUTORUN=1
		export OPENKE_ROOT PRINTER_DATA_CONFIG_SEED S02OPENKE_NAMESPACE_NO_AUTORUN
