...
#
# Usage: sh tests/openke-multi-printer-profile-tests.sh
...
PROFILES_SRC="$REPO_ROOT/scripts/build/overlay/opt/openke-seeds/printer_profiles"
...
echo "=== Test 4: S02openke-namespace First-Boot USB Override ==="
NS_WORK="$WORK/ns_test"
mkdir -p "$NS_WORK/usr/data/openke" "$NS_WORK/boot"
echo "creality-ender3-v3-se" > "$NS_WORK/boot/openke-profile.txt"

(
    export OPENKE_ROOT="$NS_WORK/usr/data/openke"
    export PRINTER_DATA_CONFIG_SEED="$REPO_ROOT/scripts/build/overlay/opt/printer_data/config"
    export PRINTER_DATA_CONFIG_MARKER="$OPENKE_ROOT/system/printer-data-config-seeded.json"
    export PRINTER_PROFILES_SEED="$PROFILES_SRC"
    export PRINTER_PROFILES_MARKER="$OPENKE_ROOT/system/printer-profiles-seeded.json"
    export S02OPENKE_NAMESPACE_NO_AUTORUN=1
    . "$S02_SCRIPT"
    start
) >/dev/null 2>&1

if [ -f "$NS_WORK/usr/data/openke/printer_data/config/printer.cfg" ]; then
    pass "S02 seeded initial printer_data/config successfully"
else
    fail "S02 failed to seed initial printer_data/config"
fi

if [ -f "$NS_WORK/usr/data/openke/system/active-profile.json" ]; then
    pass "S02 created active-profile.json metadata"
else
    fail "S02 missing active-profile.json metadata"
fi

if [ -f "$NS_WORK/usr/data/openke/printer_profiles/creality-ender3-v3-ke/profile.json" ]; then
