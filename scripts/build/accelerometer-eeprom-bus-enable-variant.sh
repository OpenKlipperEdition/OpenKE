...
# implementation. Fix: sck/mosi/miso are plain, non-inverted output/input
# lines here - GPIO_ACTIVE_HIGH. cs-gpios keeps ACTIVE_LOW, both because
# that matches stock's raw value AND because the ADXL345 (like the
# overwhelming majority of SPI slaves) is genuinely chip-select-active-low
# in hardware. Pull-config (INGENIC_GPIO_NOBIAS) is unchanged on all four
...
case "$VARIANT" in
	FIX0|FIX1) ;;
	*) echo "ERROR: unknown variant $VARIANT (must be FIX0 or FIX1)" >&2; exit 1 ;;
esac

[ -f "$DTS" ] || { echo "ERROR: $DTS not found" >&2; exit 1; }
[ -f "$FRAGMENT" ] || { echo "ERROR: $FRAGMENT not found" >&2; exit 1; }

# Strip any existing marker-wrapped blocks first.
