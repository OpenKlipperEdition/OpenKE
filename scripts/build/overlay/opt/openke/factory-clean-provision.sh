#!/bin/sh
#
# OpenKE Clean-Update + Virgin Baseline mission, Phase 4 (2026-08-08):
# on-demand factory-clean provisioning. Archives (never destroys)
# OpenKE-owned persistent state, then leaves the namespace exactly as a
# genuinely new device's would be - S04openke-factory-seed re-seeds
...
# under $OPENKE_ROOT itself - it must remain available and correct even
...
#                       (OpenKE's own generation/known-good/activation
...
APPS="$OPENKE_ROOT/apps"
ENVS="$OPENKE_ROOT/envs"
SYSTEM="$OPENKE_ROOT/system"
LOCKDIR="$OPENKE_ROOT/updates/locks"
BACKUP_ROOT="$OPENKE_ROOT/factory-clean-backups"
...
NAMESPACE_SCRIPT="${NAMESPACE_SCRIPT:-/etc/init.d/S02openke-namespace}"
...
under /usr/data/openke to a timestamped backup directory, then recreates
...
archive_and_reset() {
	...
	mkdir -p "$backup_dir"
	log "archiving current OpenKE-owned state to $backup_dir"

	for tree in apps envs system; do
		src="$OPENKE_ROOT/$tree"
		if [ -e "$src" ]; then
	...
	log "REBOOT NOW to actually provision fresh: S04openke-factory-seed will re-seed klipper/moonraker/mainsail from this image's own archives, S04openke-migrate will record the new baseline generation, and services will start clean."
}
