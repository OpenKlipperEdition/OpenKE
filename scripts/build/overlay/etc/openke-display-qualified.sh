...
# /usr/data/openke/display-qualified.conf format, its atomic writer, its
...
# own offline test suite (tests/openke-display-qualified-tests.sh) - one
...
: "${NDQ_CONFIG_DIR:=/usr/data/openke}"
...
: "${NDQ_SLEEP_WAKE_INITD:=/etc/init.d/S98openke-display-sleep-wake-controller}"
...
ndq_log() {
	echo "openke-display-qualified: $1"
}
