...
# ==========================================================================
#
# Sourced by S98openke-display-sleep-wake-controller. Also sourced
# directly by tests/openke-display-sleep-wake-controller-tests.sh so
...
# /etc/openke-display-qualified.sh and /etc/openke-camera-idle-
...
# way OPENKE_CAMERA_IDLE_POLL_INTERVAL already is in this codebase's
# other background polling controller.
OPENKE_DISPLAY_SLEEP_WAKE_POLL_INTERVAL="${OPENKE_DISPLAY_SLEEP_WAKE_POLL_INTERVAL:-0.2}"

ndq_swc_log() {
	echo "openke-display-sleep-wake-controller: $1" >&2
}
...
# tests/openke-display-qualified-tests.sh already established for these
...
ndq_swc_run_loop() {
	...
	while true; do
		...
		asleep=${result%% *}
		last_count=${result##* }
		sleep "$OPENKE_DISPLAY_SLEEP_WAKE_POLL_INTERVAL"
	done
}
