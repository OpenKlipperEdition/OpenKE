...
# under /usr/data/openke, adds a real active-print/partial-upload check
...
# mounts.
#
# Invoked by /etc/init.d/S45openke-cleanup at boot (backgrounded, does
...
LOG="$OPENKE_ROOT/maintenance/retention.log"
...
# outside $OPENKE_ROOT. Never deletes anything under a target this
# rejects.
...
clean_rotated_logs() {
	find "$OPENKE_ROOT/printer_data/logs" "$OPENKE_ROOT/maintenance/logs" \
...
clean_old_config_backups() {
	find "$OPENKE_ROOT/backups/printer_config" -maxdepth 1 -type f -mtime +7 2>/dev/null | \
...
clean_abandoned_staging() {
	find "$OPENKE_ROOT/updates/staging" -mindepth 1 -maxdepth 1 -mtime +1 2>/dev/null | while read -r f; do
		name=$(basename "$f")
...
start() {
	mkdir -p "$OPENKE_ROOT/maintenance"
	wait_for_clock_sync
