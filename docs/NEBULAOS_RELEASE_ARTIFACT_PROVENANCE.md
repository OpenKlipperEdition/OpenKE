...
| Origin (2026-08-07+) | **Fixed.** `GUPPYSCREEN_REPO`/`GUPPYSCREEN_PIN` in `manifests/dependencies.conf` identify the exact `OpenKlipperEdition/GuppyScreen` commit; `00-fetch-vendor-sources.sh` verifies that checkout, and `04-cross-compile-app-stack.sh` reuses matching tracked MIPS binaries or rebuilds them when absent/mismatched. The old hashes above are a historical snapshot only — a real build produces different (but source-traceable) bytes; `05-final-build.sh`'s manifest records both the source commit (`git_commit_guppyscreen`) and the resulting binary hash (`guppyscreen_sha256`/`guppybeep_sha256`) so the two are never ambiguous. |
